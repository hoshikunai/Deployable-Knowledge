...
function createQueryGraph(
	...
	const addNode = (node: GraphNode, score?: number, result?: KnowledgeGraphMatch) => {
		...
		const chunk = node.kind === 'chunk' && node.chunkId ? chunksById.get(node.chunkId) : undefined;
		...
		nodes.set(node.id, {
			...
			relations: uniqueRelations([...(existing?.relations ?? []), ...(result?.relations ?? [])])
		});
	};
	...
	for (const result of results) {
		...
		for (const neighbor of graph
			.neighbors(graphId('chunk', result.chunkId))
			.slice(0, MAX_QUERY_NEIGHBORS_PER_CHUNK)) {
