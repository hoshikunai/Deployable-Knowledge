...
import { documentChunks } from '$lib/server/database/schema';
import {
...
export async function buildKnowledgeGraph(options: BuildOptions): Promise<BuildResult> {
	...
	const log = (text: string) =>
		(options.logger ?? console.log)(`[kg ${duration(performance.now() - started)}] ${text}`);
	await ensureTables();
	const documentIds = uniqueIds(options.documentIds ?? []);
	const chunks = limitChunks(await loadChunks(documentIds), options.chunkLimit);
	if (!chunks.length) throw new Error('No stored document chunks are available.');
	log(`loaded ${chunks.length} chunks`);

	const scope = documentIds.length ? `documents:${hash(documentIds.join('\0'))}` : '*';
	const signature = hash(
		JSON.stringify({
			...
			chunks: chunks.map((chunk) => [chunk.chunkId, chunk.documentId, hash(chunk.content)])
		})
	);
	...
	if (!options.force && String(previous.rows[0]?.signature ?? '') === signature) {
	...
	const keys = new Map(
		chunks.map((chunk) => [chunk.chunkId, chunkKeys(chunk, hash(JSON.stringify(schema)), options)])
	);
	...
	for (const chunk of pending) {
		...
		if (llmResult && glinerResult) {
			...
			try {
				const result = hasUsableText(chunk.content)
					? await reconcileExtractions(chunk.content, schema, llmResult, glinerResult, options)
					: emptyExtraction();
...
export async function loadKnowledgeGraph(documentIds: string[] = []): Promise<KnowledgeGraph> {
	await ensureTables();
	...
	return graph(JSON.parse(String(build.rows[0].schema_json)) as CorpusSchema, assertions);
}
...
function resolveAssertions(
	...
	const names = rows.flatMap(({ assertion }) => [assertion.subject, assertion.object]);
	...
	return rows.map(({ chunk, assertion }) => {
		const subject = canonical.get(normalize(assertion.subject)) ?? assertion.subject;
...
function graph(schema: CorpusSchema, assertions: GraphAssertion[]): KnowledgeGraph {
	const entities = new Map<string, { name: string; type: string; aliases: string[] }>();
	for (const assertion of assertions) {
		for (const [name, type, raw] of [
			[assertion.subject, assertion.subjectType, assertion.provenance.rawSubject],
...
async function readCache(): Promise<Map<string, string>> {
	...
	return new Map(rows.rows.map((row) => [String(row.cache_key), String(row.result_json)]));
}

async function writeCache(rows: Array<{ key: string; value: unknown }>): Promise<void> {
	await batch(
		rows.map((row) => ({
...
function limitChunks(chunks: GraphChunk[], limit?: number): GraphChunk[] {
	...
	return Array.from({ length: count }, (_, index) => chunks[Math.round(index * step)]);
}
