...
const DEFAULT_MODEL =
	process.env.KNOWLEDGE_GRAPH_TRIPLET_MODEL ?? process.env.OLLAMA_MODEL ?? 'llama3.2:3b';
...
function parseRelation(item: Record<string, unknown>): GlinerRelation | null {
	...
	const relation = normalizeRelation(
		String(item.relation ?? item.predicate ?? item.type ?? 'RELATED_TO')
	);
	const evidence = String(item.evidence ?? item.sentence ?? '')
		.replace(/\s+/g, ' ')
		.trim();
...
function warnFallback(error: unknown): void {
	...
	console.warn(
		`Ollama triplet extractor unavailable; falling back to TypeScript extractor. ${message}`
	);
}
