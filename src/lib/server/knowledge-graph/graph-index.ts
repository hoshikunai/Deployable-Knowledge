...
import { loadKnowledgeGraphSnapshot, saveKnowledgeGraphSnapshot } from './graph-snapshot';
	...
	await graphRegistry.build(
		...
		async () => {
			...
			if (latestScope.buildScope.signature !== scope.buildScope.signature) {
				throw new Error(
					'Documents changed while the Knowledge Graph was building. Build it again.'
				);
			}
...
async function resolveGraphScope(requestedDocumentIds: string[] = []): Promise<ResolvedGraphScope> {
...
async function resolveChunkGraphScope(requestedChunkIds: string[]): Promise<ResolvedGraphScope> {
	const chunkIds = normalizeChunkIds(requestedChunkIds);
	if (!chunkIds.length) {
		return {
			documentRows: [],
	...
	const chunkRows: IndexedChunk[] = rows
		.filter((row) => row.chunkType !== 'IMAGE' || isUsefulImageText(row.content))
		.map((row) => ({
			chunkId: row.chunkId,
...
async function constructKnowledgeGraph(scope: ResolvedGraphScope): Promise<KnowledgeGraphIndex> {
	...
	if (scope.chunkRows) {
		...
		for (const row of chunkRows) {
			...
			if (row.chunkType === 'IMAGE' && !isUsefulImageText(row.content)) {
				continue;
			}
