...
export function selectHybridSeedCandidates(
	...
	const finiteScores = ranked.map((candidate) => candidate.rawScore).filter(Number.isFinite);
	...
	return ranked
		.map((candidate, index) => {
			...
			const scoreValue = Number.isFinite(candidate.rawScore) ? candidate.rawScore : minimumScore;
			// When every reranker score is tied, rank is the only useful discriminator.
			const normalizedScore =
				scoreRange > Number.EPSILON ? (scoreValue - minimumScore) / scoreRange : rankScore;
			const score = clamp01(normalizedScore * 0.7 + rankScore * 0.3);

			return createCandidate(candidate.node, 'hybrid', score, candidate.chunkId);
		})
...
export function selectExactEntitySeedCandidates(
	...
	for (const node of graph.nodes.values()) {
		...
		candidates.push(
			createCandidate(node, 'entity-exact', equalsQuery ? 1 : 0.95, undefined, node.label)
		);
	}
...
export function mergeGraphSeedCandidates(
	...
	for (const candidate of candidateGroups.flat()) {
		...
		for (const source of candidate.sources) {
			...
			existing.sourceScores[source] = Math.max(existing.sourceScores[source] ?? 0, incomingScore);
		}
...
function sortSources(sources: readonly GraphSeedSource[]): GraphSeedSource[] {
	return [...new Set(sources)].sort((left, right) => SOURCE_ORDER[left] - SOURCE_ORDER[right]);
}
