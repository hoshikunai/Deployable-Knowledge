...
function containsQueryLabel(text: string, label: string): boolean {
	...
	return new RegExp(`(?<![A-Za-z0-9])${escaped}(?![A-Za-z0-9])`, isAcronym ? '' : 'i').test(text);
}
...
function properNounPhrases(text: string): string[] {
	...
	return unique(
		matches
			.map((match) => cleanCandidateLabel(normalizeLabel(match)))
			.filter((match) => {
				...
				return words.some(
					(word) => /^[A-Z0-9]{2,}$/.test(word) || /^[A-Z][a-zA-Z0-9/-]+$/.test(word)
				);
			})
	);
}
...
function repeatedTechnicalPhrases(text: string): string[] {
	const terms = tokenize(text).filter(
		(term) => term.length > 2 && !STOP_WORDS.has(term) && !LOW_VALUE_LABELS.has(term)
	);
...
function inferKind(label: string): string {
	...
	if (
		/\b(protocol|procedure|process|workflow|instruction|manual|standard|guideline|algorithm|phase|phases|care)\b/i.test(
			label
		)
	)
		return 'protocol';
	if (
		/\b(treatment|tourniquets?|dressings?|gauze|bandages?|splints?|needle|decompression|airway|antibiotics?|medication|drug|dose|administer|control|evacuation|resuscitation)\b/i.test(
			label
		)
	)
		return 'treatment';
	if (
		/\b(injury|injuries|hemorrhage|bleeding|trauma|wound|wounds|shock|hypothermia|pain|infection|fracture|burn|burns|casualty|casualties|pneumothorax|concussion)\b/i.test(
			label
		)
	)
		return 'condition';
	if (/\b(system|platform|software|database|model|engine)\b/i.test(label)) return 'system';
	if (/\b(command|office|department|agency|unit|wing|group|squadron|center|centre)\b/i.test(label))
		return 'organization';
	if (/\b(aircraft|device|tool|equipment|radio|sensor|weapon|vehicle)\b/i.test(label))
		return 'technology';
	return 'concept';
}
...
function isUsefulEntity(label: string): boolean {
	...
	if (
		words.length > 1 &&
		words.filter((word) => LOW_VALUE_LABELS.has(word.toLowerCase())).length > words.length / 2
	)
		return false;
...
function hasBadBoundaryWord(words: string[]): boolean {
	...
	const last = words.at(-1)?.toLowerCase();
	return Boolean(
		(first && LEADING_CONNECTORS.has(first)) || (last && TRAILING_CONNECTORS.has(last))
	);
}
...
function isStrongSingleWord(label: string): boolean {
	...
	return /\b(injur(?:y|ies)|hemorrhage|tourniquets?|airway|trauma|casualt(?:y|ies)|evacuation|protocol|algorithm|antibiotics?|dressings?|gauze|bleeding|shock|hypothermia|pneumothorax|decompression)\b/i.test(
		label
	);
}
